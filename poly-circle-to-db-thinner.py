...
def check_dimensions(center_x, center_y, radius, grid_points):
    ...
    for i in range(num_points):
        ...
        # Condition 1: Check raw differences
        if (
            (1 <= delta_x <= 8 and 1 <= delta_y <= 2) or
            (1 <= delta_x <= 2 and 1 <= delta_y <= 8) or
            delta_x == 0 or delta_y == 0 or delta_x == delta_y
        ):
            continue

        # Condition 2: Check gcd-simplified differences
        if delta_x > 0 and delta_y > 0:
            common_divisor = gcd(delta_x, delta_y)
            simplified_x = delta_x // common_divisor
            simplified_y = delta_y // common_divisor
            if (
                (1 <= simplified_x <= 8 and 1 <= simplified_y <= 2) or
                (1 <= simplified_x <= 2 and 1 <= simplified_y <= 8)
            ):
                continue
...
def save_results_to_database(results, odd_center_val):
    conn = sqlite3.connect("results-thinner.db")
    c = conn.cursor()
